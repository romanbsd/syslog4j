...
public abstract class AbstractSyslog implements SyslogIF {
    @Serial private static final long serialVersionUID = 2632017043774808264L;
    ...
    protected final List<SyslogBackLogHandlerIF> notifiedBackLogHandlers = new ArrayList<>();
    ...
    public void setBackLogStatus(boolean backLogStatus) {
        if (this.backLogStatus != backLogStatus) {
            synchronized (this.backLogStatusSyncObject) {
                if (!backLogStatus) {
                    for (SyslogBackLogHandlerIF backLogHandler : this.notifiedBackLogHandlers) {
    ...
    protected String prefixMessage(String message, String suffix) {
        ...
        return ((ident == null || ident.trim().isEmpty()) ? "" : (ident + suffix)) + message;
    }
    ...
    protected void write(SyslogMessageProcessorIF messageProcessor, int level, String message)
            throws SyslogRuntimeException {
        String header = messageProcessor.createSyslogHeader(this.syslogConfig.getFacility(),
                level,
                this.syslogConfig.getLocalName(),
                this.syslogConfig.isSendLocalTimestamp(),
                this.syslogConfig.isSendLocalName());
        ...
        if (mLength <= availableLen) {
            ...
            while (left > 0) {
                ...
                boolean doSplitEndText =
                        splitBeginText != null && (firstTime || (left > (availableLen - splitBeginText.length)));
                ...
                if (actualAvailableLen < 0) {
                    throw new SyslogRuntimeException(
                            "Message length < 0; recommendation: increase the size of maxMessageLength");
                }

                byte[] data = messageProcessor.createPacketData(h,
                        m,
                        pos,
                        actualAvailableLen,
                        doSplitBeginText ? splitBeginText : null,
    ...
    protected String modifyMessage(int level, String message) {
        List<SyslogMessageModifierIF> _messageModifiers = this.syslogConfig.getMessageModifiers();
        ...
        for (SyslogMessageModifierIF messageModifier : _messageModifiers) {
    ...
    public void backLog(int level, String message, String reason) {
        ...
        List<SyslogBackLogHandlerIF> backLogHandlers = this.syslogConfig.getBackLogHandlers();

        for (SyslogBackLogHandlerIF backLogHandler : backLogHandlers) {
    ...
    public AbstractSyslogWriter createWriter() {
        ...
        try {
            newWriter = (AbstractSyslogWriter) clazz.getDeclaredConstructor().newInstance();
