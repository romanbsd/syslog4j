...
public class SyslogParameterTest extends TestCase {
    ...
    public void checkMessageModifier(AbstractSyslogConfigIF syslogConfig) {
    ...
    protected void checkBackLogHandler(AbstractSyslogConfigIF syslogConfig) {
        ...
        List<SyslogBackLogHandlerIF> list = new ArrayList<>();
        list.add(handler);
    ...
    public void testTcpNetSyslogConfigCreate() {
        AbstractNetSyslogConfig config;

        config = new TCPNetSyslogConfig(SyslogConstants.FACILITY_AUTH);

        checkBackLogHandler(config);
        checkMessageModifier(config);

        assertSame(config.getSyslogClass(), TCPNetSyslog.class);
        ...
        assertFalse(tcpConfig.isUseDaemonThread());
    ...
    public void testUdpNetSyslogConfigCreate() {
        ...
        assertSame(config.getSyslogClass(), UDPNetSyslog.class);
        assertEquals(SyslogConstants.FACILITY_AUTH, config.getFacility());
        ...
        assertEquals(SyslogConstants.INCLUDE_IDENT_IN_MESSAGE_MODIFIER_DEFAULT,
                config.isIncludeIdentInMessageModifier());
        ...
        assertEquals(
                SyslogConstants.SPLIT_MESSAGE_BEGIN_TEXT_DEFAULT,
                new String(config.getSplitMessageBeginText()));
        ...
        assertEquals(
                SyslogConstants.SPLIT_MESSAGE_END_TEXT_DEFAULT,
                new String(config.getSplitMessageEndText()));
    ...
    public void testTcpNetSyslogServerConfigCreate() {
        ...
        assertEquals(SyslogConstants.TCP_MAX_ACTIVE_SOCKETS_BEHAVIOR_DEFAULT,
                tcpServerConfig.getMaxActiveSocketsBehavior());
        tcpServerConfig.setMaxActiveSocketsBehavior(TCPNetSyslogServerConfigIF.MAX_ACTIVE_SOCKETS_BEHAVIOR_REJECT);
        assertEquals(TCPNetSyslogServerConfigIF.MAX_ACTIVE_SOCKETS_BEHAVIOR_REJECT,
                tcpServerConfig.getMaxActiveSocketsBehavior());
        ...
        assertEquals(1, tcpServerConfig.getEventHandlers().size());
        assertSame(handler2, tcpServerConfig.getEventHandlers().get(0));
        ...
        assertFalse(tcpServerConfig.isUseDaemonThread());
    ...
    public void testSSLTCPNetSyslogConfigCreate() {
        TCPNetSyslogConfig config;

        config = new SSLTCPNetSyslogConfig();
        assertSame(config.getSyslogWriterClass(), SSLTCPNetSyslogWriter.class);

        config = new SSLTCPNetSyslogConfig(SyslogConstants.FACILITY_AUTH);
        assertSame(config.getSyslogClass(), SSLTCPNetSyslog.class);
        assertEquals(SyslogConstants.FACILITY_AUTH, config.getFacility());
    ...
    public void testPooledTCPNetSyslogConfigCreate() {
        PooledTCPNetSyslogConfig config;

        config = new PooledTCPNetSyslogConfig();
        assertSame(config.getSyslogWriterClass(), TCPNetSyslogWriter.class);

        config = new PooledTCPNetSyslogConfig(SyslogConstants.FACILITY_AUTH);
        assertSame(config.getSyslogClass(), PooledTCPNetSyslog.class);
        assertEquals(SyslogConstants.FACILITY_AUTH, config.getFacility());
    ...
    public void testSyslog4jBackLogHandler() {
        ...
        try {
            syslog4j.log(Syslog.getInstance("udp"),
                    SyslogConstants.LEVEL_INFO,
                    "Log4j BackLog Test Message - IGNORE",
                    "really");
            fail();
        } catch (SyslogRuntimeException sre) {
            //
    ...
    public void testMultipleSyslogConfigCreate() {
        ...
        assertSame(protocols, config.getProtocols());
        assertEquals(0, config.getProtocols().size());

        try {
            config.setCacheHostAddress(true);
            fail();
        } catch (SyslogRuntimeException sre) {
        }
        try {
            config.setCharSet("foo");
            fail();
        } catch (SyslogRuntimeException sre) {
        }
        try {
            config.setFacility(SyslogConstants.FACILITY_AUTH);
            fail();
        } catch (SyslogRuntimeException sre) {
        }
        try {
            config.setFacility("foo");
            fail();
        } catch (SyslogRuntimeException sre) {
        }
        try {
        ...
        try {
            config.addBackLogHandler(null);
            fail();
        } catch (SyslogRuntimeException sre) {
        }
        try {
            config.addMessageModifier(null);
            fail();
        } catch (SyslogRuntimeException sre) {
        }
        try {
            config.insertBackLogHandler(0, null);
            fail();
        } catch (SyslogRuntimeException sre) {
        }
        try {
            config.insertMessageModifier(0, null);
            fail();
        } catch (SyslogRuntimeException sre) {
        }
        try {
            config.removeAllBackLogHandlers();
            fail();
        } catch (SyslogRuntimeException sre) {
        }
        try {
            config.removeAllMessageModifiers();
            fail();
        } catch (SyslogRuntimeException sre) {
        }
        try {
            config.removeBackLogHandler(null);
            fail();
        } catch (SyslogRuntimeException sre) {
        }
        try {
            config.removeMessageModifier(null);
            fail();
        } catch (SyslogRuntimeException sre) {
        }
        ...
        assertSame(MultipleSyslog.class, config.getSyslogClass());
    ...
    public void testStructuredSyslogMessage() {
        ...
        assertNotEquals("Wrong Class", m1);
        assertEquals(m1, m1);
        assertEquals(m2, m2);
    ...
    public void testStructuredSyslogEvent() {
        ...
        String message =
                "<165>1 2003-10-11T22:14:15.003Z hostname appname process-id message-id [id@1234 test1=\"test2\"] test3";

        StructuredSyslogServerEvent event =
                new StructuredSyslogServerEvent(message.getBytes(), message.length(), localhost);
        ...
        StructuredSyslogServerEvent event2 =
                new StructuredSyslogServerEvent(message2.getBytes(), message2.length(), localhost);
    ...
    public void testUnixSocketSyslogConfigParameters() {
        ...
        assertNull(syslogConfig.getHost());
